 ...
 * \ingroup tcp
 *
 * \brief (abstract) base class of all MyTcpSockets
 *
 * This class exists solely for hosting MyTcpSocket attributes that can
 * be reused across different implementations.
 */
class MyTcpSocket : public Socket
{
public:
  /**
   * Get the type ID.
   ...
   */
  static const char* const TcpStateName[MyTcpSocket::LAST_STATE];
 ...
 */
typedef void (* TcpStatesTracedValueCallback)(const MyTcpSocket::TcpStates_t oldValue,
                                              const MyTcpSocket::TcpStates_t newValue);
