...
private:
  ...
  double 	  m_finishTime;  //!< the time of receiving all the packets
  double 	  m_startTime;   //!< the time of receiving the first packet
  int 		  m_isFirst;     //!< judge if the first packet is receiced
  uint32_t  m_senderNumber; //!< number of client
  uint32_t  m_maxBytes;    //!< the maxBytes of each client send
